...
export default class EthereumHDWallet extends HDWallet{
    ...
    constructor(secret = null, address=null) {
        ...
        this.transactions = [];
        this.usedAddresses = [];
        this.plugins = [{ name: 'Compound', class: Compound}, {name: 'Uniswap', class: Uniswap}, {name: 'Maker', class: Maker}, {name: 'Livepeer', class: Livepeer}, {name: 'Dydx', class: Dydx}]
    ...
    getERC20BalanceWei(contractAddress) {
        ...
        const idx = this.findTokenIdx(contractAddress);
        console.log('this.tokens[idx]', this.tokens[idx])
    ...
    async export() {
        ...
        console.log('this.portfolio', this.Uniswap.portfolio) 
    ...
    async getAccountNetWorth() {
        ...
        this.Compound.portfolio.forEach( t => {
            const token = this.findToken(t.contractAddress);
            if( token.price && token.price.rate && t.supplyAmount) {
                networth += (parseFloat(token.price.rate) * parseFloat(t.supplyAmount.text));
            }
        })
        ...
        this.Dydx.portfolio.forEach( t => {
            const token = this.findToken(t.contractAddress);
            if( token.price && token.price.rate && t.balance) {
                networth += (parseFloat(token.price.rate) * parseFloat(t.balance));
            }
        })
    ...
    async loadTokensList(pruneCache=false) {
        ...
        return fetch(`https://api.ethplorer.io/getAddressInfo/${this.getAddress()}?apiKey=scwf7425sUxrtI106`)
            ...
            .then( async data => {
                ...
                this.tokens = orderBy(tokens, ['symbol'], ['asc']);

                this.tokens.forEach(async t => {
                    await this.fetchERC20Balance(t.contractAddress)
                });
